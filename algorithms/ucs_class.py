...
class UcsPuzzle:
    ...
    def build_node(self, state, parent_node=None, action=None, node_depth=0, cost=0):
        return {
            'node state': state,
    ...
    def explore_node(self, node):
        ...
        if blank_tile_pos not in [0, 1, 2]:
            adjacent_nodes.append(self.build_node(self.swap_tiles(node['node state'], blank_tile_pos, blank_tile_pos - 3), node, 'Up', node['node depth']+1, node['cost']+1))
        if blank_tile_pos not in [0, 3, 6]:
            adjacent_nodes.append(self.build_node(self.swap_tiles(node['node state'], blank_tile_pos, blank_tile_pos - 1), node, 'Left', node['node depth']+1, node['cost']+1))
        if blank_tile_pos not in [2, 5, 8]:
            adjacent_nodes.append(self.build_node(self.swap_tiles(node['node state'], blank_tile_pos, blank_tile_pos + 1), node, 'Right', node['node depth']+1, node['cost']+1))
        if blank_tile_pos not in [6, 7, 8]:
            adjacent_nodes.append(self.build_node(self.swap_tiles(node['node state'], blank_tile_pos, blank_tile_pos + 3), node, 'Down', node['node depth']+1, node['cost']+1))
        return adjacent_nodes
    
    def uniform_cost_search(self, write_file):
        exp_n = 0
        pop_n = 0
        gen_n = 1
        max_fs = 1
        ...
        while len(frontier) > 0:
            ...
            if (current_node['node state'] == self.target_node).all():
                    return exp_n, gen_n, pop_n, max_fs, current_node['node depth'], current_node['cost'], self.find_solution_path(current_node)
            ...
            for child in adjacent_nodes:
                gen_n += 1
                frontier.append((child['cost']+1, child))
...
if __name__ == "__main__":
    ...
    if dump:
        ...
        puzzle = UcsPuzzle(np.array(start), np.array(target))
        result = puzzle.uniform_cost_search(trace_dump_path)
        ...
        puzzle = UcsPuzzle(np.array(start), np.array(target))
        result = puzzle.uniform_cost_search(-1)
        exp_n, gen_n, pop_n, max_fs, soln_depth, soln_cost, soln_path = result
        if soln_path != -1:
            moves, cost= puzzle.track_solution_cost(soln_path)
            puzzle.print_styled(exp_n, gen_n, pop_n, max_fs, soln_depth, cost, moves)
