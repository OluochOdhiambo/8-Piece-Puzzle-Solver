...
class GreedyPuzzle:
    ...
    def compute_manhattan_distance(self, node_state):
        node_state = np.reshape(node_state, (3, 3))
        target_state = np.reshape(self.target_node, (3, 3))
        distance = 0
        for i in range(3):
            for j in range(3):
                if node_state[i][j] != 0:
                    x, y = np.where(target_state == node_state[i][j])
                    distance += abs(i - x[0]) + abs(j - y[0])
        return distance
    
    def greedy_search(self, write_file):
        exp_n = 0
        gen_n = 0
        pop_n = 0
        max_fs = 0
        ...
        while len(frontier) > 0:
            ...
            if (current_node['node state'] == self.target_node).all():
                ...
                return exp_n, gen_n, pop_n, max_fs, current_node['node depth'], cost, solution_path
            ...
            for child in self.explore_node(current_node):
                gen_n += 1
                child['heuristic'] = self.compute_manhattan_distance(child['node state'])
    ...
    def print_styled(self, exp_n, gen_n, pop_n, max_fs, curr_d, cost, soln_moves):
...
if __name__ == "__main__":
    ...
    if dump:
        ...
        puzzle = GreedyPuzzle(np.array(start), np.array(target))
        result = puzzle.greedy_search(trace_dump_path)
