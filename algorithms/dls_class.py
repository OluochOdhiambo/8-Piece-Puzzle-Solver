...
class DlsPuzzle:
    ...
    def depth_limit_search(self, max_depth, write_file):
        exp_n = 0
        gen_n = 0
        pop_n = 0
        max_fs = 0
...
if __name__ == "__main__":
    ...
    if dump:
        ...
        puzzle = DlsPuzzle(np.array(start), np.array(target))
        result = puzzle.depth_limit_search(max_depth, trace_dump_path)
        ...
        puzzle = DlsPuzzle(np.array(start), np.array(target))
        result = puzzle.depth_limit_search(max_depth, -1)
        exp_n, gen_n, pop_n, max_fs, soln_depth, soln_path = result
        if soln_path != -1:
            moves, cost= puzzle.track_solution_cost(soln_path)
            puzzle.print_styled(exp_n, gen_n, pop_n, max_fs, soln_depth, cost, moves)
